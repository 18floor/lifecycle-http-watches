    ...
    render() {
        ...
        return (
            <div className="clock-wrapper">
                <span>{clock.cityName}</span>
                <div className="clock-watches">
                    <div className="clock-hand clock-hand-hours"
                         style={{transform: `rotate(${hours * 360 / 12}deg)`}}></div>
                    <div className="clock-hand" style={{transform: `rotate(${minutes * 360 / 60}deg)`}}></div>
                    <div className="clock-hand clock-hand-seconds"
                         style={{transform: `rotate(${seconds * 360 / 60}deg)`}}></div>
                    <button className="clock-btn" onClick={() => handleDelete(clock.id)} aria-label="delete clock">x
                    </button>
                </div>
